...
const MangerTaskTMS = ({ role }) => {
  ...
  const fetchTaskComments = async (taskId) => {
    ...
    try {
      const response = await axios.get(`https://api-emsdev-be-epb9fbg0e7ewese6.southindia-01.azurewebsites.net/task/${taskId}/comments`);
  ...
  const fetchTasks = async () => {
    try {
      ...
      const res = await axios.get(`https://api-emsdev-be-epb9fbg0e7ewese6.southindia-01.azurewebsites.net/manager/emp/task/${managerId}/`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const taskTypeRes = await axios.get(`https://api-emsdev-be-epb9fbg0e7ewese6.southindia-01.azurewebsites.net/api/task-types/unique-names`);
        setTaskType(taskTypeRes.data.taskTypes || []);
        
        const projectRes = await axios.get(`https://api-emsdev-be-epb9fbg0e7ewese6.southindia-01.azurewebsites.net/api/projects/unique-names/${user._id}`);
