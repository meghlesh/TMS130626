...
const { initializeWhatsApp, sendWhatsAppMessage, sendWhatsAppFile,  isWhatsAppReady } = require("../whatsapp/whatsappClient");
...
async function sendMonthlyLeaveBalance() {
  try {
    ...
    //     { 
    //       role: "hr",
    //       email: "rushikesh.takale@creativewebsolution.in"
    //     }
    //   ]
    // });
    ...
    const individualUsers = await User.find({ 
      isDeleted: { $ne: true },
      role: { $in: ["Team_Leader", "employee", "IT_Support", "manager","hr"] }
    });
